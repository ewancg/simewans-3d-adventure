...
Error Graphics::beginFrame(Window &t_windowIn) {
  ...
  for (SDL_GPUCommandBuffer *&item : m_commandBuffers) {
...
Error Graphics::attachWindow(Window &t_windowIn) {
  ...
  if (!SDL_ClaimWindowForGPUDevice(m_device, win)) {
    return {GPU_WINDOW_CLAIM, SDL_GetError()};
  }
  if (!SDL_SetGPUSwapchainParameters(m_device, win, SDL_GPU_SWAPCHAINCOMPOSITION_SDR,
...
Error Graphics::getWindowSwapchainTexture(Window &t_windowIn, SDL_GPUTexture *&t_textureOut) {
  ...
  if (commandBuf == nullptr) {
    return {GET_WINDOW_SWAPCHAIN_TEXTURE, "no rendering command buffer provided"};
  }

  auto *window = t_windowIn.getRawHandle();
  if (window == nullptr) {
    return {GET_WINDOW_SWAPCHAIN_TEXTURE, "no subject window provided"};
  }
  // Acquire the swapchain texture respective to the input window
...
// We should constrain this, I don't know what t_data
template <typename T>
Error Graphics::uploadData(GPUBuffer &t_buf, T *&t_data, uint32_t t_size, bool t_cycle) {
  void *mappedBuffer = SDL_MapGPUTransferBuffer(m_device, m_uploadBuffer, t_cycle);
